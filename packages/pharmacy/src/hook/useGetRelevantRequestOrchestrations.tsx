...
import { isPharmacyAbleToDispense, requestIsFromOrchestration } from "core/src/util/matchingUtil";
...
export const useGetRelevantRequestOrchestrations = () => {
  ...
  const relevantOrchestrations = useMemo(() => {
    const validRequests = requests.filter(
      (v) => isPharmacyAbleToDispense([v], selectedPharmacy) && perms.canCompleteMedication(v)
    );
